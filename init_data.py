...
class InitData:
    ...
    # Building the InitData objects, extracted from the initial Excel tables
    ...
    # Get information about the required column of the initial Excel table
    def getColumn(self,columnName):
        return self.df[columnName].tolist()
    
    # Multiply the prices by a reducing factor due to stock splits
    ...
    # Get the cut index list that define the different days
    def getCut(self):
    ...
    # Fill the missing minutes
    def fillMissingMinutes(self,symbol): 
        cut=self.getCut()
        
        # Find the missing minutes, between the given start and end times
        m=len(cut)
        for j in range(m-2, -1, -1):
            for i in range(cut[j+1]-2,cut[j]-1,-1):
                ...
                if t_i_bis-t_i>1:
                        ...
                        # Insertion of the new data records
...
# Extracting the Excel tables that represent AAPL, AMZN, DJIA and BTC
aaplData = InitData("C:/Users/Utilisateur/Documents/Mirror/Études/ICM_2A/Stage 2A/UiO/3. Le stage/2. Ressources externes/A. Initial HFFD/AAPL.csv")
amznData = InitData("C:/Users/Utilisateur/Documents/Mirror/Études/ICM_2A/Stage 2A/UiO/3. Le stage/2. Ressources externes/A. Initial HFFD/AMZN.csv")
djiaData = InitData("C:/Users/Utilisateur/Documents/Mirror/Études/ICM_2A/Stage 2A/UiO/3. Le stage/2. Ressources externes/A. Initial HFFD/DJIA.csv")
btcData  = InitData("C:/Users/Utilisateur/Documents/Mirror/Études/ICM_2A/Stage 2A/UiO/3. Le stage/2. Ressources externes/A. Initial HFFD/BTC.csv")

# Getting the dataframe from the underlaying symbol
def getData(symbol):
...
# Gettting the ticker name from the underlaying symbol
...
# Getting the earliest trading time among all of the trading days for one choosen asset/index
...
# Getting the latest trading time among all of the trading days for one choosen asset/index
...
# Getting the number of the trading days for one choosen asset/index            
def getNumberTradingDays(symbol):
   return len(getData(symbol).getCut())-1

# Getting the maximum time gap between two data records in the same trading day for asset/index symbol
...
# Getting the maximum time gap between the first data records in a trading day 
# and the expected start of this trading day for a given asset/index
...
# Getting the maximum time gap between the last data records in a trading day 
# and the expected end of this trading day for a given asset/index
...
# Getting the number of almost complete trading days for BTC reported to
# a given margin
...
aaplData.df.to_csv("C:/Users/Utilisateur/Documents/Mirror/Études/ICM_2A/Stage 2A/UiO/3. Le stage/3. Mon travail/Filled data/aaplData.csv", index=False)
amznData.df.to_csv("C:/Users/Utilisateur/Documents/Mirror/Études/ICM_2A/Stage 2A/UiO/3. Le stage/3. Mon travail/Filled data/amznData.csv", index=False)
djiaData.df.to_csv("C:/Users/Utilisateur/Documents/Mirror/Études/ICM_2A/Stage 2A/UiO/3. Le stage/3. Mon travail/Filled data/djiaData.csv", index=False)
btcData.df.to_csv("C:/Users/Utilisateur/Documents/Mirror/Études/ICM_2A/Stage 2A/UiO/3. Le stage/3. Mon travail/Filled data/btcData.csv", index=False)
