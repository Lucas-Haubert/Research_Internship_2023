...
# For each symbol, prices at times t_0,...,t_n are observed
nValues = {"AAPL": 31955,"AMZN": 28794,"DJIA": 33701,"BTC": 167487}

# Getting the log-prices for each asset
...
# Time lag between two observations (from the trading time line)
def delta(symbol):
    n=nValues[symbol]
    return 1/n
...
# Pre-averaging window size in the "noise" case
def k(symbol):
...
# Dictionary with values [minimum log(price),maximum log(price)] for each symbol and 
# for each price type
dictionaryMinMax={}
symbols=["AAPL","AMZN","DJIA","BTC"]
...
# Estimators of the weighted sum Phi and the conditional variance Sigma
...
#[Adapter ce paragraphe une fois que le code est plié]
