...
def plotEstimatorHistoryLag_1(symbol,priceType):
    ...
    # List [S(0),S(1),...,S(n)], len n
    S=data.getColumn(priceType) 
    
    # Array [S(0) S(1) ... S(n)], len n
    S=np.array(S) 
    
    # Array [log(S(0)) log(S(1)) ... log(S(n))], len n
    logS=np.log(S) 
    
    # Array [log(S(1))-log(S(0)) ... log(S(n))-log(S(n-1))], len n-1
    diff=np.diff(logS) 
    
    # Array [(log(S(1))-log(S(0)))^2 ... (log(S(n))-log(S(n-1)))^2], len n-1
    STild=diff**2 
    
    # Array [S_tild(0) S_tild(0)+S_tild(1)...somme des S_tild(k)]
    cumsumSTild=np.cumsum(STild) 
...
def plotEstimatorHistoryLag_m(symbol,priceType,m):
    ...
    # List [S(0),S(1),...,S(N)], len N+1
    S=data.getColumn(priceType) 
    
    # Array [S(0) S(1) ... S(N)], len N+1
    S=np.array(S) 
    
    # Array [log(S(0)) log(S(1)) ... log(S(N))], len N+1
    logS=np.log(S)  
    
    n=len(logS)
    numWindows=n//m  # Number of windows based on m

    # Calculate squared differences within each window
    diffM=np.diff(logS[:m*numWindows].reshape(numWindows,m).T,axis=1).T.reshape(-1)
    
    STildM=diffM**2

    # Calculate cumulative sums of squared differences
...
# Test and execution time measure
startTime=time.time()
